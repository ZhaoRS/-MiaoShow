...
@class ZRSCarouselView;

typedef void(^ClickBlock)(NSInteger index);

//pageControl的显示位置
typedef NS_ENUM(NSUInteger, PageControlPosition) {
    PageControlPositionNone,            //默认值 == PositionBottomCenter
...
typedef NS_ENUM(NSUInteger, PageChangeMode) {
    PageChangeModeDefault,              //轮播滚动
    PageChangeModeFade,                 //淡入淡出
};

@protocol ZRSCarouselViewDelegate <NSObject>
 ...
 *  其他属性都有默认值，可不设置
 */
@interface ZRSCarouselView : UIView

/*
 这里没有提供修改占位图片的接口，如果需要修改，可直接到.m文件中
 搜索"ZRSPlaceholder"替换为你想要显示的图片名称，或者将原有的占位
 图片删除并修改你想要显示的图片名称为"ZRSPlaceholder"。
 不需要占位图片的请将[UIImage imageNamed:@"ZRSPlaceholder"]
 ...
 */
@property (nonatomic, assign) PageChangeMode changeMode;
...
*  代理，用来处理图片的点击
*/
@property (nonatomic, weak) id<ZRSCarouselViewDelegate> delegate;
